...
export default function StressPracticePage() {
  ...
  const [isInitialWordLoadPending, setIsInitialWordLoadPending] = useState(false);
  ...
  // Calculate next review date based on history
  const calculateNextReview = (
    history: { correct: boolean; date: Date }[],
    today: Date
  ) => {
    if (history.length === 0) return today;

    const streak = calculateStreak(history);
    return computeNextReview(streak, new Date(history[0].date));
  };
  ...
  data,
  error,
  isLoading, // Используем isLoading из trpc для определения загрузки данных с бэкенда
  isError,
  refetch: stressPracticeRefetch,
} = trpc.GetPracticeWords.useQuery(
  { type: "STRESS" },
  {
    onError: (error) => {
      console.error("Failed to fetch words:", error);
      setisLoadingState(false); // Убедимся, что состояние загрузки UI обновляется при ошибке
    },
  }
);
useEffect(() => {
  if (isInitialWordLoadPending && words.length > 0) {
    loadNextWord(); // `loadNextWord` теперь будет использовать актуальное состояние `words`
    setIsInitialWordLoadPending(false); // Сбрасываем флаг, начальная загрузка выполнена
  }
}, [isInitialWordLoadPending, words]);
  // Record practice attempt
  const recordPractice = trpc.RecordPractice.useMutation({
    onSuccess: () => {
      // Don't refetch words, just update local state
      // stressPracticeRefetch(); - Remove this line
    },
  });

  // Fetch words only once on component mount
  useEffect(() => {
    if (data && !isLoading && isLoadingState) { // Используем !isLoading от trpc и isLoadingState для UI
      const transformedWordsData = data.words.map((word) => ({
        id: word.id,
        word: word.word,
        accentIdx: word.accentIdx,
        correctStreak: calculateStreak(word.history),
        wrongCount: calculateWrongCount(word.history),
        nextReview: calculateNextReview(word.history, today),
        retired: calculateStreak(word.history) >= 4,
      }));
  
      setWords(transformedWordsData);
      setisLoadingState(false); // Завершаем состояние загрузки UI
  
      if (transformedWordsData.length > 0) {
        setIsInitialWordLoadPending(true); // Сигнализируем, что слова установлены и нужна начальная загрузка
      } else {
        // Если с бэкенда не пришло слов
        setCurrentWord(null);
        setAllLearned(true); // Все "выучено", так как учить нечего
      }
    } else if (!isLoading && !data && isLoadingState) {
      // Случай, когда загрузка завершена, но данных нет (например, пустой ответ от бэкенда не через ошибку)
      setWords([]);
      setisLoadingState(false);
      setCurrentWord(null);
      setAllLearned(true);
    }
  }, [data, isLoading, isLoadingState, today]);

  // Get due words and set the first one as current on mount
  // useEffect(() => {
  //   if (!isLoadingState && words.length > 0) {
  //     loadNextWord();
  //   }
  // }, [isLoadingState, words]);
...
const handleSelect = (idx: number): void => {
  ...
  // Record practice attempt to backend without triggering refetch
  if (currentWord.id) {
  ...
  // Важно: НЕ вызываем loadNextWord() здесь
};
                ...
                <div className="flex justify-center w-full mt-6">
                  {requireRetry ? (
                    ...
                    <button
                      ...
                      className="px-8 py-3 bg-indigo-600 text-white font-medium rounded-xl shadow-md hover:bg-indigo-700 active:bg-indigo-800 transition-all duration-200 ease-in-out mx-auto focus:outline-none focus:ring-0 transform hover:scale-105"
                      disabled={!feedback} // Кнопка активна только после получения обратной связи
                    >
                      Следующее слово
                    </button>
                  )}
